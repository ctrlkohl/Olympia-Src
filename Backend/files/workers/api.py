...
async def test(request): 
    return sanic.response.json("u really thought u will find something here?")
...
@app.route('/fortnite/api/cloudstorage/user/<accid>/ClientSettings.Sav', methods=['POST', 'GET', 'PUT'])
async def test(request, accid: str):
...
async def test(request):
    return sanic.response.json([{"channels":{"client-events":{"states":[{"state":{"seasonNumber":13,"seasonTemplateId":"AthenaSeason:athenaseason13","seasonBegin":"0001-06-23T04:00:00Z","seasonEnd":"9999-07-23T04:00:00Z","seasonDisplayedEnd":"9999-07-23T04:00:00Z"}}]}},"currentTime": "9998-06-23T04:00:00Z"}
])
...
@app.route('/affiliate/api/public/affiliates/slug/jko', methods=['POST', 'GET'])
async def test(request):
...
@app.route('/affiliate/api/public/affiliates/slug/edik1045', methods=['POST', 'GET'])
async def test(request):
  ...
  return sanic.response.json({
  	...
  	"id": "aabbccddeeff11223344556677889900",
  	"slug": "edik1045",
  	"displayName": "edik1045",
  	"status": "ACTIVE",
  	"verified": 'true'
  		
  })

@app.route('/affiliate/api/public/affiliates/slug/reksely%20lox', methods=['POST', 'GET'])
async def test(request):
